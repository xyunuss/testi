(function ($) {
	...
	var halo = {
		...
		menuSidebarMobile: function () {
			...
			const menuSidebarMobileOpen = () => {
				closeOverlays();
				if ($("body").hasClass("mobilemenu_open")) {
					console.log("close menu");
					$(".menu-heading-mobile .halo-sidebar-close").trigger("click");
				} else {
					console.log("open menu");
					$body.addClass("menu_open");
					$body.addClass("mobilemenu_open");
				...
				entries.forEach((entry) => {
					if (entry.isIntersecting) {
						...
						if (!$block.hasClass("ajax-loaded")) {
							halo.doAjaxProductTabs(linkActive.data("href"), activeTab.find(".loading"), activeTab.find(".products-load"), $block.attr("sectionid"), limit, $block);
						}
		...
		swapHoverVideoProductCard: function () {
			if (window.innerWidth > 1200) {
				...
				$(".product-item .card").mouseleave(function () {
					var chil = $(this).find("video:not(:first-child)");
					var _chil = $(this).find("video").get(0);
					if (chil.length > 0) {
						_chil.pause();
					}
		...
		initColorSwatch: function () {
			...
			$doc.on("click", ".card .swatch-label", (event) => {
				...
				if (mediaList.length > 0) {
					...
					if (newImage) {
						product.find(".card-media img:nth-child(1)").attr("srcset", newImage).attr("data-srcset", newImage);
					}
				}
		...
		checkPreOrderOfVariant: function (selectedVariant, productAction, productJson) {
			...
			const updatePreOrderText = (productAction, selectedVariant, inventoryQuantity) => {
				...
				if (selectedVariant.inventory_management == null) {
					...
					productAction.text(window.variantStrings.preOrder).prop("disabled", false).addClass("button--pre-untrack");
				} else {
					...
					if (inventoryQuantity > 0) {
						...
						if (window.notify_me.show) {
							productAction.removeClass("is-visible").addClass("is-notify-me").prop("disabled", false);
						} else {
							productAction.removeClass("is-visible").addClass("is-soldout").prop("disabled", true);
						}
					}
					...
					if (productAction.hasClass("is-soldout") || productAction.hasClass("is-notify-me")) {
						...
						productAction.text(hasQuickShopPanel ? window.variantStrings.add : window.variantStrings.addToCart).prop("disabled", false);
					}
				}
			};
						...
						swatch.each((idx, elt) => {
							...
							if (optionSoldout == undefined) {
								if (optionUnavailable == undefined) {
									...
									item.removeClass("unavailable available").addClass("soldout").find(".single-action").attr("data-variant-id", optionUnavailable.title);
						...
						swatch.each((idx, elt) => {
							...
							if (optionSoldout == undefined) {
								if (optionUnavailable == undefined) {
									...
									item.removeClass("unavailable available").addClass("soldout").find(".single-action-selector").attr("data-variant-id", optionUnavailable.title);
									item.find('input[type="radio"]').prop("disabled", false);
								}
							} else {
								item.removeClass("soldout unavailable").addClass("available");
					...
					() => {
						...
						if (window.after_add_to_cart.type == "cart") {
							...
							Shopify.getCart((cartTotal) => {
								...
								$body.find("[data-cartitems]").attr("data-cartitems", cartTotal.item_count);
		...
		initSidebarCart: function () {
			...
			$body.on("click", ".background-overlay", (event) => {
				if ($body.hasClass("cart-sidebar-show") && !$body.hasClass("edit-cart-show") && !$body.hasClass("term-condition-show")) {
					if ($(event.target).closest("#halo-cart-sidebar").length === 0 && $(event.target).closest("[data-cart-sidebar]").length === 0 && $(event.target).closest("[data-edit-cart-popup]").length === 0 && $(event.target).closest("[data-warning-popup]").length === 0 && $(event.target).closest("[data-term-condition-popup]").length === 0) {
						$body.removeClass("cart-sidebar-show");
					}
				}
			});
		...
		updateSidebarCart: function (cart) {
			if (!$.isEmptyObject(cart)) {
				...
				const $cartLoading =
					'<div class="loading-overlay loading-overlay--custom hidden">\
				...
				$.ajax({
					...
					complete: function () {
						...
						$body.find("[data-cartitems]").attr("data-cartitems", cart.item_count);
		...
		changeStateforContactForm(formMessage, error) {
			...
			if (error) {
				...
				$("html, body").animate({ scrollTop: formMessage.offset().top - window.innerHeight / 2 }, 700);
			}
		},
		...
		initQuickView: function () {
			...
			$(".background-overlay")
				...
				.on("click.closeQuickView", (e) => {
					if ($body.hasClass("quick-view-show") && !$body.hasClass("cart-sidebar-show") && !$body.hasClass("ask-an-expert-show") && !$body.hasClass("size-chart-show") && !$body.hasClass("compare-color-show") && !$body.hasClass("term-condition-show")) {
						$body.removeClass("quick-view-show");
					}
				});
		},
			...
			$.ajax({
				...
				complete: function () {
					...
					items.each((index, element) => {
						...
						itemInput.attr("id", `quickView-${itemId}`).attr("name", `quickView-${itemId}`);
		...
		productImageGallery: function ($scope) {
			var sliderNav = $scope.find(".productView-navV1"),
			...
			if (!sliderFor.hasClass("slick-initialized") && !sliderNav.hasClass("slick-initialized")) {
				...
				if (sliderNav.hasClass("productView-nav-gallery")) {
					var sliderNav2 = $scope.find(".productView-navV1.productView-nav-gallery"),
					...
					var sliderNav2 = $scope.find(".productView-navV1.productView-horizontal-tabs"),
						show = 2,
						rows = 1;
		...
		productVideoGallery: function ($scope) {
			...
			if (videoThumbnailLen) {
				const $imageWrapper = $scope.find(".productView-image-wrapper"),
					...
					sliderNav = $scope.find(".productView-navV1");
		...
		initWishlist: function () {
			if (window.wishlist.show) {
				halo.setLocalStorageProductForWishlist();
				console.log("halo.setLocalStorageProductForWishlist()");
				...
				$doc.on("click", "[data-wishlist]", (event) => {
					...
					(index = wishlistList.indexOf(handle)), (wishlistContainer = $("[data-wishlist-container]"));
					wishlistLayout = wishlistContainer.attr("data-wishlist-layout");

					if (!$target.hasClass("wishlist-added")) {
						$target.addClass("wishlist-added").find(".text").text(window.wishlist.added);
						...
						$target.removeClass("wishlist-added").find(".text").text(window.wishlist.add);
						if (wishlistContainer.length > 0) {
						...
						if (wishlistContainer.length > 0) {
							...
							if (wishlistList.length > 0) {
								...
								console.log("wishlistList.length > 0");
		...
		setProductForWishlist: function (handle) {
			...
			if (index >= 0) {
				...
				item.removeClass("wishlist-added").find(".text").text(window.wishlist.add);
			}
		},
		...
		checkScrollLayoutForRecenlyViewed() {
			...
			if (RecentlyViewedSection && RecentlyViewedSection.classList.contains("products-flex")) {
				const RecentlyViewedObserver = new MutationObserver((mutationList, observer) => {
					...
					if (mutationRecentlyViewed.type == "attributes" && mutationRecentlyViewed.target.classList.contains("recently-viewed-loaded")) {
	...
	$(".header__icon--cart").on("click", function (e) {
		e.preventDefault();
		//e.stopPropagation();
		$body.addClass("cart-sidebar-show");
		halo.initSidebarCart();
