		...
		checkPreOrderOfVariant: function (selectedVariant, productAction, productJson) {
			...
			const updatePreOrderText = (productAction, selectedVariant, inventoryQuantity) => {
				...
				if (selectedVariant.inventory_management == null) {
					...
					productAction.text(window.variantStrings.preOrder).prop("disabled", false).removeClass("is-notify-me").addClass("button--pre-untrack");
				} else {
			...
			if (enable) {
				...
				const filter = (list) => {
					switch (pageLayout) {
							...
							if (window.innerWidth > 767) {
								...
								if (sliderNavHTML.length) {
									list.split(",").forEach((element) => {
										if (sliderNavHTML.includes(element.replace(/\./g, ""))) checkList++;
									});
									if (checkList == 0) {
						...
						default:
							sliderNav.slick("slickFilter", list).slick("refresh").addClass("slick-filter");
					}
				...
				const setImageForMetafields = (list, unfilter) => {
					...
					array.forEach((element) => {
						slidesToKeep.push(`.filter-${element.toLowerCase().replace(/[^A-Z0-9]/gi, "-")}`);
					});
